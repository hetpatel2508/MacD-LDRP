...
export default function CoffeeBeverages() {
    ...
    React.useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response1 = await fetch('http://localhost:6868/product/4', {
                    method: 'GET',
                    headers: {
                ...
                const response2 = await fetch('http://localhost:6868/product/5', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiaWF0IjoxNzM2MDc0MDkyfQ.V5oHrUQ0DGA4fqfnMGTsWU7VleELyOUk9DeJuVBhasw`,
