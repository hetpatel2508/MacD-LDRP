...
export default function McSavers() {
    ...
    React.useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:6868/meal/18', {
                    method: 'GET',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    setFilteredProducts(data.data); // Set filteredProducts initially
                }
            } catch (error) {
    ...
    ${selectedCategory === category || (selectedCategory === 'NonVeg' && category === 'Non-Veg')
            ...
            <div className='w-[92%] h-full grid grid-cols-5 text-center gap-6 mt-5'>
                {filteredProducts.length > 0 ? (
                    ...
                    <div className='col-span-5 text-xl font-bold text-gray-500'>
                        No products found
                    </div>
                )}
            </div>
